...
US-004's issue left its title at `(round 1)` while the body had moved to `(round 2)`.
...
is. (PR #28 / US-013, 2026-08-09: the agent filed a complete, well-verified findings
issue and then stopped — no verdict file, no `agent-fix` label. The workflow's fallback
had no real signal to act on and defaulted to `needs-human` for a PR that had no
blocking findings at all. Do not let this be step 8 you meant to get to; it is part of
step 7, in the same turn as filing or updating the issue.)
