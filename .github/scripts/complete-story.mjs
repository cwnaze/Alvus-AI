...
// commits again. US-004 burned three review rounds and three fix runs in half an hour
// riding that loop. So the review's own bookkeeping is reconstructed here instead, at
// merge time, which is the first moment writing to main is free again.
//
...
// each round but leaves the title at whatever round opened the issue. US-004's issue #7
// read "(round 1)" in its title while its body was already on round 2.
const roundOf = (i) => {
  const m =
